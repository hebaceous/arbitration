 ...
 * 自定义刷新主页数据(前台实现删除)
 */
function refreshData_index(json){
	if(json.statusCode == DWZ.statusCode.ok){
		// 左侧审批删除
		$("tr[mark_remove="+json.id+"]").remove();

		// 右侧监控更新
		var $tr = $("tr[mark_update="+json.id+"]");
		var status = parseInt($tr.attr("status"));
		if(status == 23){
			$tr.remove();
		}
		var text = null;
		switch (status) {
			case 1 : text = "审批仲裁申请（副主任）"; break;
			case 2 : text = "审批仲裁申请（中心主任）"; break;
			case 3 : text = "联络科长指定秘书"; break;
			case 4 : text = "送达回证中--立案后"; break;

			case 5 : text = "处理回证和文书--立案后"; break;
			case 6 : text = "处理回证和文书--立案后"; break;
			case 7 : text = "处理回证和文书--立案后"; break;

			case 8 : text = "选定仲裁员"; break;
			case 9 : text = "审批仲裁员选定表（副主任）"; break;
			case 10 : text = "审批仲裁员选定表（中心主任）"; break;
			case 11 : text = "审批仲裁员选定表（仲裁委主任）"; break;

			case 13 : text = "送达回证--组庭后"; break;
			case 14 : text = "等待收到回证--组庭后"; break;
			case 15 : text = "上传开庭笔录"; break;
			case 16 : text = "下达裁决书"; break;
			case 17 : text = "审批裁决书（副主任）"; break;
			case 18 : text = "审批裁决书（中心主任）"; break;
			case 19 : text = "审批裁决书（仲裁委主任）"; break;
			case 20 : text = "送达回证--裁决书后"; break;
			case 21 : text = "等待收到回证--裁决书后"; break;
			case 22 : text = "案件归档"; break;
			default:
				break;
		}
		$tr.children("td:eq(4)").text(text);

		// 直接刷新案件状态列表
//		$("#caseing").load("case/caseing");

		// 判断是否刷新流程中的数据
//		if(json.refreshId){
//			$("#"+json.refreshId).load(json.refreshUrl);
//		}
	}
}
